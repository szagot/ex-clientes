...
if ( is_numeric( $clientId ) && $clientId >= 0 && $clientId < count( $clientes ) ) {
    ...
    $pessoa = ( $cliente instanceof Clientes\ClienteJuridico ) ? 'Jurídica' : 'Física';
    ?>
    <p>
        ...
        <?= $cliente->getLogradouro() ?>, <?= $cliente->getNumero() ?> - <?= $cliente->getCidade() ?>
        /<?= $cliente->getUf() ?><br>
...
<table cellpadding="5" cellspacing="1" border="1">
    ...
    foreach ( $clientes as $index => $cliente ) {
        ...
        <tr class="cliente">
            ...
            <td><?= $pessoa ?></td>
            <td>
                <a href="?clientId=<?= $index ?>&ordem=<?= $ordecacaoAsc ? 'asc' : 'desc' ?>"><?= $cliente->getNome() ?></a>
            </td>
        </tr>
        <?php
    }
